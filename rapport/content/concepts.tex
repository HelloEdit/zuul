...
Apeller \code{values)} sur un enum (dans notre cas précédent \code{Day.values} retourne alors toutes les constantes possibles de l'enum (toujours dans notre cas, nos constantes sont \code{MONDAY}, \code{TUESDAY}, \code{WEDNESDAY}, \code{THURSDAY}, \code{FRIDAY}, \code{SATURDAY} et \code{SUNDAY}).
